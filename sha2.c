...
int scanhash_sha256(int thr_id, uint32_t *pdata, const uint32_t *ptarget,
										uint32_t max_nonce, unsigned long *hashes_done)
{
	uint32_t data[64] __attribute__((aligned(128)));
	uint32_t hash[8] __attribute__((aligned(32)));
	...
	memcpy(data, pdata, 64); // Prepare the initial data block

	do
	{
		data[19] = ++n;									 // Update nonce in the data array
		sha256_init(hash);							 // Initialize SHA256 state
		sha256_transform(hash, data, 0); // Perform the single SHA256

		// Check if the hash is below the target
		if (swab32(hash[7]) <= Htarg)
		{
			pdata[19] = data[19]; // Store the valid nonce
			if (fulltest(hash, ptarget))
			{ // Verify full PoW target
				*hashes_done = n - first_nonce + 1;
				return 1;
			}
		}
...
int scanhash_sha256ET10(int thr_id, uint32_t *pdata, const uint32_t *ptarget,
												uint32_t max_nonce, unsigned long *hashes_done)
{
	uint32_t data[64] __attribute__((aligned(128)));
	uint32_t hash[8] __attribute__((aligned(32)));
	uint32_t n = pdata[19] - 1;
	const uint32_t first_nonce = pdata[19];
	const uint32_t Htarg = ptarget[7] * 10;
	...
	do
	{
		...
		// Check if the hash is below the scaled target
		if (swab32(hash[7]) <= Htarg)
		{
			pdata[19] = data[19]; // Store the valid nonce
			if (fulltest(hash, ptarget))
